...
const ReviewComponent = ({ info }) => {
  ...
  return (
    <div className="w-[424px] space-y-8 rounded-xl bg-white px-6 py-12 shadow-md">
      ...
      <p className="text-base text-center leading-6 text-textGray">
        {info.description || "No review available."}
      </p>
    </div>
  );
};
              ...
              <Marquee gradient gradientColor="#eeefff" gradientWidth={120}>
                <div className="mx-6 flex items-center gap-6">
                  {reviews.map((item, index) => (
                    <ReviewComponent key={item.id || index} info={item} />
                  ))}
                </div>
              </Marquee>
              <Marquee
